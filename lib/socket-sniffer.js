...
var saveData = (function () {
  ...
  return function (data, fileName) {
    ...
    Object.keys(data).map((hid) => {
      ...
      if (typeof hdataMessages[0] !== 'undefined' && hdataMessages[0].indexOf('Hand #') !== -1 && hdataMessages[1].indexOf(tableId) !== -1) {
...
function parseGameState(message, site) {
  ...
  if (message.t === 'GameState') {
    ...
    const handId = gi
    const messageHandData = handData?.[handId] || {messages: []}
    ...
    if (events) {
      ...
      events.forEach((event, eventIdx) => {
        ...
        if(actionIndex && eT === 9) {
          if(!messageHandData?.flop && !messageHandData?.hole) {
            messageHandData['hole'] = true
            messageHandData.messages = messageHandData.messages.concat([`*** HOLE CARDS ***`])
            handData[handId] = messageHandData
          ...
          if(playerAction === 'did something') {
            console.log("what did " + playerName + " do?")

...
class PokerHand {
  ...
  finalizeHand = async () => {
    ...
    const dateString = today.toISOString().replaceAll('-','/').replace('T', ' ').replace(/(\..*)$/,'') + ' UTC'

    const line1 = `PokerStars Hand #${this.handId}: ${this.gameType} ${this.potType} (${this.smallBlindAmount}/${this.bigBlindAmount}) - ${dateString} [${dateString}]`
    const line2 = `Table '${this.site}_${this.tableId}' 6-max (Real Money) Seat #${this.dealer + 1} is the button`
